...
$stmt = $pdo->prepare('SELECT idproduct, name, main_img FROM product ORDER BY idproduct DESC LIMIT 6');
$stmt->execute();
...
<body class="bg-pink-50 text-gray-800">
  ...
  <!-- Hero Section -->
  <section class="text-center py-16 bg-pink-300 text-white">
    <h1 class="text-5xl font-bold mb-4">Welcome to <img src="./assets/logo/logo.png" class="w-40 inline-block -ml-4 -mr-12" alt="Logo">ommee!</h1>
    <p class="text-xl mb-6">Your one-stop shop for beauty and quality products</p>
    <a href="pages/login.php" class="bg-white text-pink-600 font-semibold px-6 py-3 rounded-full shadow hover:bg-pink-100 transition">Browse Products</a>
  </section>

  <!-- Featured Products -->
  <section class="py-12 px-6 md:px-20 max-w-7xl mx-auto">
    <h2 class="text-3xl font-bold text-pink-600 mb-8 text-center">Featured Products</h2>
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      <?php if (!empty($products)): ?>
        <?php foreach ($products as $product): ?>
          <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition overflow-hidden cursor-pointer">
            <img 
              src="data:image/jpeg;base64,<?= base64_encode($product['main_img'])?>" 
              alt="<?= htmlspecialchars($product['name']) ?>" 
              class="w-full h-48 object-cover"
            >
            <div class="p-4">
              <h3 class="text-lg font-semibold text-gray-800 mb-2"><?= htmlspecialchars($product['name']) ?></h3>
              <p class="text-gray-600 text-sm mb-3">A stylish item that's perfect for your collection.</p>
            </div>
          </div>
        <?php endforeach; ?>
      <?php else: ?>
        <p class="text-center text-gray-500 italic col-span-full">No products available at the moment.</p>
      <?php endif; ?>
    </div>
  </section>
